...
# Class for composing multiple data augmentation transforms
class Compose:
...
# class for random horizontal flip transform with adjustments for bounding boxes, masks, and keypoints
...
# Class for transform to convert a PIL image to a tensor
...
# Class for transform to convert image data type to a specified dtype
...
# Class for random IoU-based cropping of images
class RandomIoUCrop(nn.Module):
    ...
    def forward(
        ...
        while True:
            # Sample an option for minimum IoU overlap
            idx = int(torch.randint(low=0, high=len(self.options), size=(1,)))
            min_jaccard_overlap = self.options[idx]
            if min_jaccard_overlap >= 1.0:  # If IoU is larger than or equal to 1, keep the image as-is
                return image, target
...
# Class for random zoom-out transformation
...
#  Class for random photometric distortions
class RandomPhotometricDistort(nn.Module):
    def __init__(
        ...
        super().__init__()
        # Initialize various color jitter transformations
...
# Class for fixed-size cropping of images
...
class RandomShortestSize(nn.Module):
    def __init__(
        ...
        super().__init__()
        # Initialize the minimum size as a list even if it's provided as an integer
    ...
    def forward(
        ...
        # Randomly choose a minimum size from the provided list
        min_size = self.min_size[torch.randint(len(self.min_size), (1,)).item()]

        # Calculate the scaling factor 'r' to resize the image
        ...
        if target is not None:
            ...
            if "masks" in target:
                # Resize masks accordingly with nearest-neighbor interpolation
                target["masks"] = F.resize(
                    target["masks"], [new_height, new_width], interpolation=InterpolationMode.NEAREST
                )
