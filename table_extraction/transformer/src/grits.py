...
def objects_to_cells(
    ...
    tokens_in_table = [
        ...
        if eval_utils.iob(token["bbox"], table_bbox) >= 0.5
    ]
...
def adjacency_metrics(true_adjacencies, pred_adjacencies):
    ...
    if len(true_adjacencies) > 0:
        recall = (sum(true_c.values()) - sum((true_c - pred_c).values())) / sum(
            true_c.values()
        )
    else:
        recall = 1
    if len(pred_adjacencies) > 0:
        precision = (sum(pred_c.values()) - sum((pred_c - true_c).values())) / sum(
            pred_c.values()
        )
    else:
        precision = 1
...
def align_cells_outer(true_cells, pred_cells, reward_function):
    ...
    for row_idx in range(1, len(true_cells) + 1):
        for col_idx in range(1, len(pred_cells) + 1):
            reward = align_1d(
                true_cells[row_idx - 1], pred_cells[col_idx - 1], reward_function
            )
            diag_score = scores[row_idx - 1, col_idx - 1] + reward
            same_row_score = scores[row_idx, col_idx - 1]
            same_col_score = scores[row_idx - 1, col_idx]
...
def factored_2dlcs(true_cell_grid, pred_cell_grid, reward_function):
    true_row_nums, pred_row_nums, row_score = align_cells_outer(
        true_cell_grid, pred_cell_grid, reward_function
    )
    ...
    for true_row_num, pred_row_num in zip(true_row_nums, pred_row_nums):
        for true_column_num, pred_column_num in zip(true_column_nums, pred_column_nums):
            score += reward_function(
                true_cell_grid[true_row_num][true_column_num],
...
def compute_metrics(
    ...
    true_cell_dilatedbbox_grid = np.array(
        output_to_dilatedbbox_grid(true_bboxes, true_labels, true_scores)
    )
    ...
    pred_cell_dilatedbbox_grid = np.array(
        output_to_dilatedbbox_grid(pred_bboxes, pred_labels, pred_scores)
    )
    ...
    (
        ...
        metrics["grits_rawloc_rowbased"],
        metrics["grits_rawloc_columnbased"],
    ) = factored_2dlcs(
    ...
    (
        ...
        true_relspan_grid, pred_relspan_grid, reward_function=eval_utils.iou
    )
...
def grits(args, model, dataset_test, device):
    ...
    for idx in range(0, max_samples):
        ...
        true_table_structures, true_cells, true_confidence_score = objects_to_cells(
            true_bboxes,
            true_labels,
            true_scores,
            page_tokens,
        ...
        if idx % 1000 == 0:
            ...
            print(
                "Total time taken for {} samples: {}".format(
                    idx, datetime.now() - st_time
                )
            )
        ...
        if args.debug:
            ...
            print(
                "Adjacency w/ blanks f-score: {}".format(
                    metrics["adjacency_withblank_fscore"]
                )
            )
            ...
            rescaled_bboxes = rescale_bboxes(
                torch.tensor(boxes[0], dtype=torch.float32), img_test.size
            )
            for bbox, label, score in zip(
                rescaled_bboxes, labels[0].tolist(), scores[0].tolist()
            ):
    ...
    print(
        "GriTS_RawLoc: {}".format(
            np.mean([result["grits_rawloc"] for result in results])
        )
    )
    print("GriTS_Loc: {}".format(np.mean([result["grits_loc"] for result in results])))
    print(
        "GriTS_Cont: {}".format(np.mean([result["grits_cont"] for result in results]))
    )
    print("GriTS_Top: {}".format(np.mean([result["grits_top"] for result in results])))
    print(
        "Adjacency f-score: {}".format(
            np.mean([result["adjacency_nonblank_fscore"] for result in results])
        )
    )
    print(
        "Adjacency w/ blanks f-score: {}".format(
            np.mean([result["adjacency_withblank_fscore"] for result in results])
        )
    )
        ...
        "GriTS_RawLoc: {}".format(
            np.mean(
                [
                    ...
                    if not math.isnan(result["grits_rawloc"])
                ]
            )
        )
    )
